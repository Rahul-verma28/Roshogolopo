...
const cartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action: PayloadAction<CartItem>) => {
      const existingItem = state.items.find(
        (item) =>
          ...
          item.weightOption === action.payload.weightOption
      );
    ...
    removeFromCart: (
      state,
      action: PayloadAction<{ productId: string; weightOption: string }>
    ) => {
    ...
    applyCoupon: (
      ...
      console.log("Applying coupon:", action.payload);
    ...
    calculateTotals: (state) => {
      ...
      if (state.appliedCoupon) {
        console.log("Calculating discount with coupon:", state.appliedCoupon.discount, state.appliedCoupon.value);
    ...
    builder
      ...
      .addCase(validateCart.fulfilled, (state, action) => {
        ...
        action.payload.items.forEach((validationResult: any) => {
          const item = state.items.find(
            (item) => item.productId === validationResult.productId
          );
        ...
        state.items = state.items.filter((item) => {
          const validation = action.payload.items.find(
